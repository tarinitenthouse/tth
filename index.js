...
// for handling cors
const cors_options = {
    origin: "https://tth.my-style.in", // public URL
...
app.get('/', (req, res) => {
    res.send('Hello Welcome to my Server point ?')
})
...
app.post('/api/post/:slug', async (req, res) => {
    if (req.params.slug == "send_otp") {
    ...
    else if (req.params.slug == "post_request") {
        const user_data = await req.body;
        ...
        if (user_details.length >= 1) {
            ...
            if (email_status == 200) {
                ...
                await request.save();
                res.send({ status: "request_saved" });
            }
            else if (email_status == 400) {
                res.send({ status: "request_saved_failed" });
            }
        }
    }
        ...
        const user_data = await req.body;
        const user_requests = await Request.find({ user_cookie_id: user_data.user_cookie_id }); // return a array which contain the user_cookie_id value 
